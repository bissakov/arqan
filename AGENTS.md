...
```sh
make              # build bin/arqan and bin/arqan-highlight
make run          # build and run
make fmt          # format src/, highlight/, tests/unit/ and tests/fuzz/
make check-format # verify formatting without writing
make check-globals # reject loose globals
...
make lint         # cppcheck, sparse and flawfinder
make fuzz         # build the libFuzzer targets in tests/fuzz/
make clean        # remove build/, bin/ and dist/
```

Run `make fmt`, `make check-globals` and `make test EXTRA_CFLAGS=-Werror` after
every change under `src/`. Warnings are failures: CI builds with `-Werror` on
top of `-Wall -Wextra -Wpedantic -Wconversion`, and a plain `make` does not
add it.
...
failure. Check a suspected flake with `make test T="--repeat 5"`.
...
- Unity build: `src/main.c` includes every `.c` file. Add new modules there,
  not to the Makefile.
- `src/agent.h` is the only cross-module header. The one exception is
  `src/highlight_protocol.h`, which `agent.h` includes and the `highlight/`
  helper shares.
- No `malloc` or `free`. Use the startup arenas: `persist` for session-lifetime
  data, `scratch` for per-turn data.
- Allocation failure is expected. Check `arena_alloc`, `buf_ok` and collection
...
- Read owned files through `file_read`. Write updates through a temporary file
  and a rename.
- Code identifiers use `agent`/`agent_`/`AGENT_`/`Agent`. Text a user sees,
  including messages, paths and environment variables, uses `AGENT_NAME` and
  `AGENT_ENV_PREFIX`, never a literal product name. Rename the product with
...
Leave `g_persist`, `g_scratch` and `g_screen` as they are: they are the static
backing stores of the arenas, not state. Do not thread an `App *` through every
function: thousands of call sites, unreviewable, and it lands on the paths
`bench-guard` protects.

## Module map

- `core.c`: arenas, strings and buffers, logging, time, file reads.
- `width.c`: code point column widths. Generated by `scripts/gen-width.py`; do
  not edit it.
- `json.c`: JSON parser and serializer.
- `http.c`: libcurl requests, streaming SSE, idle polling.
- `web.c`: the `page_fetch` and `internet_search` tools. HTML goes through
  lexbor in `vendor/lexbor/`.
...
- `secrets.c`: API key sources: the credentials file, Secret Service, `pass`,
  the macOS keychain, or a command.
- `models.c`: per-model profiles: context window and reasoning settings.
- `favorites.c`: `/model` pins.
- `catalog.c`: the model list `/model` offers, fetched when the picker opens.
- `prompt.c`: system prompt discovery and expansion.
- `cli.c`: command-line parsing.
- `ignore.c`: `.gitignore` and `.ignore` matching for the file tools.
- `tools.c`: the registry and the built-in tools.
- `mcp.c`: MCP servers over stdio. Everything a server sends is untrusted.
- `todo.c`: the todo list tool.
...
- `subagent.c`, `tasklog.c`: the read-only delegate behind the `task` tool,
  run in a worker process, and the event log it writes for its parent.
- `context.c`: the context gauge.
...
- `notify.c`: notifications when a turn ends or needs input, sent as a
  terminal escape or through `notify_command`.
- `tui.c`, `markdown.c`, `render.c`: terminal UI and transcript rendering.
- `highlight.c`: talks to the `arqan-highlight` child process. Its source is in
  `highlight/`; `highlight/queries.c` is generated by
  `scripts/embed-highlights.py`, and `vendor/tree-sitter/` is refreshed by
...
- Settings resolve through one table in `config.c`: defaults, XDG config files,
  `.arqan/config.toml` in the working directory and each directory above it
  (the nearer file wins), the state file, the active provider, `ARQAN_<NAME>`,
  then CLI options.
- A project config file is untrusted. Each `k_conf` row has a `project` flag,
  and a project file may set only rows where it is true. Rows with it false
  include `api_key`, `base_url`, `permissions`, `notify_command`, `images`,
  `telemetry` and `mcp`. Set it false for any new setting that sends data or a
  key somewhere, runs a command, or widens trust. A provider defined in a
  project file gets no API key and no key-store directive. A rejected value is
  reported and dropped, never clamped.
...
1. In one commit, update `AGENT_VERSION` and `CHANGELOG.md`, and refresh the
   golden files with `make test-update`.
